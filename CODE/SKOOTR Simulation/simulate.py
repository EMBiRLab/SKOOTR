...
robot = p.loadURDF("skootr.urdf", [0,0,0.1], initOrn,useFixedBase = 0)
...
def get_q0():
   mag = acosd((L0**2*cosd(yaw)**2*sind(pitch)**2 + L0**2*sind(yaw)**2)**(1/2)/L0)
   return mag
...
def IK(x,y):
  ...
  #print(' q1: ' + str(q1) + ' q2: ' + str(q2)+ ' x1: ' + str(x1) + ' y1: ' + str(y1)  + ' yaw: ' + str(yaw))
